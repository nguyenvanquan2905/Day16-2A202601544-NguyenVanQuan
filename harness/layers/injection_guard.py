...
def _remove_untrusted_blocks(content: str) -> str:
    while (start := content.find(BLOCK_START)) >= 0:
        end = content.find(BLOCK_END, start + len(BLOCK_START))
        suffix = "" if end < 0 else content[end + len(BLOCK_END):]
        content = content[:start] + PLACEHOLDER + suffix
    return content
...
class InjectionGuard(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        if not isinstance(result.content, str) or BLOCK_START not in result.content:
            return result
        content = _remove_untrusted_blocks(result.content)
